...
\gls{aws} können im ausgewählten Ausgabeformat dargestellt werden. Hierbei
...
Solange die \gls{cmpltapp} nicht terminiert, wird im Ausgabefenster der derzeitige Ergebnisstand im gewählten Ausgabeumfang ausgegeben. 
...
Das Ausgabeformat entscheidet über die Visualisierung im Ausgabefenster. Das Ausgabeformat hat jedoch keine Auswirkung auf das Exportieren von Ergebnissen.\\
...
Beim Wechsel vom Step-by-Step-Modus zur Komplettauswertung wird der zuletzt dargestellte \gls{lt} des Ausgabefeldes weiter ausgewertet.
...
{Der eingegebene \gls{lt} wird ausgewertet und im gewählten Ausgabeformat wie spezifiziert dargestellt.}
...
{Die IDE verhält sich wie spezifiziert (vgl. \ref{fnc:completereduce}).
 Im Ausgabefeld erscheint ein Syntaxbaum gemäß der ausgewählten Ausgabelänge (vgl. \ref{fnc:selectsize}).}
...
\begin{figure}[H]
	...
	\caption{\label{fig:tree}Wird das Ausgabeformat "Tree" ausgewählt (siehe \cref{fig:outputOptions}), so wird der eingegebene \gls{lt} als Syntaxbaum im Ausgabefeld angezeigt.}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{\label{fig:unicode}Wird das Ausgabeformat "Unicode" ausgewählt (siehe \cref{fig:outputOptions}), so wird der eingegebene \gls{lt} in Unicode im Ausgabefeld angezeigt.}
\end{figure}
