...
public class FSDTest {
	...
	public void T2() {
		...
		URL u = (URL) p.unicodeOutput()
				.find("(span/span/span/span/span/span/a[@class='gwt-Anchor abstraction clickable'])[1]");
		u.hover();
	...
	public void T4() {
		...
		// T4.7
		// not testable as there are no exercises where the solution can be copied to
		// the input
	...
	public void T7() {
		...
		p.runButton().click();
		assertEquals("\"foo.bar = \\x. x\" is not a valid name assignment (row 1, colums 1-15)",
				p.unicodeOutput().readText());
		...
		// T7.3
		// spec for binding two terms was changed to shadow earlier terms, so we test
		// that instead
	...
	public void T10() {
		...
		// spec contained error at this point: first and second are analogous to head
		// and tail in lists
