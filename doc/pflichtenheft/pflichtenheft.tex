...
\newglossaryentry{std}
{
	...
	description={Standardmäßig sind folgende Einstellungen ausgewählt:
	 ...
	 \item Darstellungsumfang: Result only (vgl. \cref{fig:outputOptions})
	 \end{itemize}}
}
...
Der Editor zur Eingabe der \glspl{lt} unterstützt Standart-Editor-Funktionen,
...
\criteriumOptional{Darstellung von Teilschritten}{crt:partial}
Die zum Erreichen der Normalform notwendigen Reduktionsschritte gemäß der ausgewählten
\gls{aws} können im ausgewählten Darstellungsformat dargestellt werden. Hierbei
...
\criteriumOptional{Schritt-für-Schritt-Reduktion}{crt:steps}
...
Das Editorfeld verfügt über Standart-Textfeld-Funktionalitäten mit dazugehörigen gängigen Tastenkombinationen:
\begin{itemize}
	\item Editieren, Markieren und Navigieren von Text im Eingabefeld
	\item Kopieren, Ausschneiden und Einfügen
	\item Undo und Redo
\end{itemize}
Außerdem verfügt das Editorfeld über \gls{st}s und \gls{mfe}.
...
Neben dem Editorfeld werden Zeilennummern angezeigt.
...
Die Reihenfolge der \glspl{beta} wird durch die derzeit aktive \gls{aws} bestimmt. Die Darstellung wird durch das derzeit aktive Darstellungsformat (\cref{fig:outputOptions}) bestimmt.\\
...
Solange die \gls{cmpltapp} nicht terminiert, wird im Darstellungsfenster der derzeitige Ergebnisstand im gewählten Darstellungsumfang ausgegeben. 
Insbesondere bei einer Auswahl der Darstellungsumfang von ``Full'' oder ``Periodically'' werden dem Nutzer regelmäßig die derzeit berechneten \gls{lt} ausgegeben.
...
\begin{itemize}
	...
	Anschließend wird der zuletzt berechnete \gls{lt} dargestellt. Es können nun keine weiteren Auswertungen am \gls{lt} vorgenommen werden. 
	Dies wird dem Nutzer durch eine entsprechende Ausgabe im Darstellungsfeld mitgeteilt.
...
Der Nutzer hat die Möglichkeit, sich Auswertungen von \gls{lt}en in Unicode darstellen zu lassen. Die Darstellung wird im Darstellungsfeld ausgegeben.\\
Jeder neu ausgegebene Auswertungsschritt wird in einer neuen Zeile dargestellt. 
...
\fulfills{crt:display}
Der Nutzer hat die Möglichkeit, sich Auswertungen von \gls{lt}en als Syntaxbaum darstellen zu lassen. Die Darstellung wird im Darstellungsfeld ausgegeben (siehe \cref{fig:tree}).\\
Jeder neu ausgegebene Auswertungsschritt wird als neuer Baum unter dem letzten Auswertungsschritt dargestellt. Hierbei wird ein \gls{lt} in folgender Weise in einen Baum übersetzt:
...
\functionality{Auswahl des Darstellungsformats}{fnc:display}
\fulfills{crt:display}
Der Nutzer kann das derzeitig aktive Darstellungsformat über ein Drop-down-Menü auswählen (\cref{fig:outputOptions}).
Das Darstellungsformat entscheidet über die Visualisierung im Darstellungsfenster. Das Darstellungsformat hat jedoch keine Auswirkung auf das Exportieren von Ergebnissen.\\
...
Die \gls{aws} bestimmt die Reihenfolge von automatischen \glspl{beta} und damit die Markierung des nächsten zu reduzierenden \gls{lt}s im Darstellungsfeld.\\
...
\functionality{Auswahl des Darstellungsumfangs}{fnc:selectsize}
\fulfills{crt:partial}
Der Nutzer kann den Darstellungsumfang über ein Drop-down-Menü auswählen (\cref{fig:outputOptions}). 
Standardmäßig ist der Darstellungsumfang ``Result only'' ausgewählt. 
Der Darstellungsumfang bestimmt die Menge an Teilergebnissen einer Auswertung, die die \gls{cmpltapp} in das Darstellungsfeld schreibt.
Der Nutzer kann den Darstellungsumfang während der \gls{cmpltapp} nicht ändern (vgl. \cref{fig:state}).
Hierbei stehen dem Nutzer folgende Optionen zur Verfügung:
\begin{itemize}
	\item Result only. Es wird nur das Ergebnis der \gls{cmpltapp} dargestellt.
	\item Shortened. Es werden nur die ersten 50 Schritte und die letzten 50 Schritte dargestellt.
	\item Full. Es wird jeder einzelne Teilschritt dargestellt, sobald er berechnet wurde.
	\item Periodically. Jeder hundertste Teilschritt wird dargestellt, sobald er berechnet wurde. 
	Der erste Schritt und das Endergebnis werden ebenfalls dargestellt.
\end{itemize}
...
wird im Darstellungsfenster eine Fehlermeldung angezeigt. Die Fehlermeldung enthält die genaue Position (Zeilen-
...
Dabei wird die Auswertung abgebrochen und der Inhalt des Darstellungsfelds erhalten, die Steuerung der Darstellung aber deaktiviert.
...
Die Darstellung berücksichtigt den Namen soweit möglich:
...
Wird das Darstellungsformat geändert, so wird die gesamte Darstellung im Darstellungsfeld entfernt.
Die entfernte Darstellung wird im neuen Darstellungsformat rekonstruiert und angezeigt.\\
...
\fulfills{crt:perma}
Die in der Adresszeile des Browsers angezeigte URL kodiert den gesamten Zustand der Sitzung. Dies schließt den Inhalt des Eingabefelds, die ausgewählten Optionen und den Inhalt des Darstellungsfelds mit ein.
...
Dieses enthält den aktuelle Inhalt des Darstellungsfelds im ausgewählten Format.
...
Beim Wechsel in den Eingabemodus werden Eingabefeld und Darstellungsfeld geleert.
...
Zeitgleich wird das Eingabefeld deaktiviert. Wenn noch Ausgaben einer früheren Auswertung im Darstellungsfeldfeld stehen, werden diese gelöscht.\\ 
Durch Drücken des Start- oder Pause-Knopfes kann der Nutzer zwischen den entsprechenden Auswertungsmodi wechseln. 
Beim Wechsel vom Step-by-Step-Modus zur Komplettauswertung wird der zuletzt dargestellte \gls{lt} des Darstellungsfeldes weiter ausgewertet.
Die Auswertungsmodi können erst durch das Drücken des Abbrechen-Knopfes verlassen werden. Es erscheint eine entsprechende Darstellung im Darstellungsfeld. Erst jetzt kann der Nutzer neue Eingaben im Eingabefeld eingeben.
...
{Ben gibt in das Eingabefeld im oberen Teil des Fensters einen gültigen \gls{lt} mit Klammern ein,
...
{Ben überprüft, dass als Darstellungsformat die Unicode-Darstellung, als Reduktionsordnung
die \gls{nr} und als Darstellungsumfang die volle Ausgabe ausgewählt sind.}
...
{Die Komplettauswertung pausiert und gibt das zuletzt berechnete Ergebnis im Darstellungsfeld aus.}
%TODO more teststeps
...
Darstellungsformat aus, begibt sich in den Schritt-für-Schritt-Modus und durchläuft einige
...
{Die IDE öffnet sich im neuen Fenster und Eingabe- und Darstellungsfeld befinden sich im gleichen Zustand wie im alten Fenster.}
...
{Eingabe und Darstellung werden entfernt.
...
{Nutzer "Bob" drückt den "Pause"-Button.}
{Die Auswertung wird angehalten, der aktuelle \gls{lt} wird in das Ausgabefenster geschrieben, die IDE wechselt in den Step-by-Step Modus.}

\teststep{Die IDE befindet sich im Step-by-Step-Modus}
{Bob drückt den "Play"-Blutton.}
{Die ID wechselt in den Komplettauswertungs-Modus, der eingegebene \gls{lt} wird ohne Aktionen des Nutzers ausgewertet.}

\teststep{Die Komplettauswertung eines \gls{lt} läuft}
{Bob drückt den "Abbrechen"-Button.}
{Die Auswertung wird abgebrochen, das Ausgabefenster wird zurückgesetzt, das Eingabefenster kann bearbeitet werden.}
...
{Alan definiert eine natürliche Zahl.
...
\begin{figure}[H]
	...
	\caption{\label{fig:exerciseMode} Wurde eine Übungsaufgabe ausgewählt, so wird die Aufgabenstellung in einem Textfeld angezeigt. Eventuell vorgegebene Eingaben werden in das Editorfenster geladen. Der Nutzer hat die Möglichkeit, das Größenverhältnis dieser beiden Fenster anzupassen. Die Musterlösung der Aufgabe wird durch Klicken eines entsprechenden Button angezeigt werden. Mit Klick auf einen weiteren Button kann der Übungsmodus beendet werden.}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{subfigure}{0.25\textwidth}
		...
		\caption{Neue Ausgabe bei Anklicken der ausgewählten \gls{lapp}.}
	\end{subfigure}
	\hspace*{\fill}
	\begin{subfigure}{0.25\textwidth}
		\centering
		\includegraphics[width=\textwidth]{img/displayTree5}
		\caption{Ausgabe bei Auswahl der Wurzel.}	
	\end{subfigure}
	\caption{Wird eine "App"-Symbol (\gls{lapp}) angeklickt, so wird dieser Teilterm als neue Ausgabe angezeigt.}
\end{figure}
