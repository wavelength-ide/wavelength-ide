...
public final class Exercises {
	...
	public static List<Exercise> all() {
		...
		String alphaReduTask = "α-conversion enables renaming variables bound by an abstraction with some restrictions to prevent the term"
				+ " from changing its structure: "
				+ "<br/> The renaming must not result in free variables becoming bound ones and only variables bound to the selected abstraction may be renamed. "
				+ "<br/> If a term can be transformed into another term only by α-renaming variables, the two terms are α-equivalent."
		...
		String order1Task = "If a λ-term contains multiple reducable expressions the question which to reduce first is answered by the applied reduction order "
				+ " one of which is the normal reduction order. "
				+ "<br/> Using this reduction order the outermost leftmost redex is reduced first. "
				+ "<br/> In respect to the tree output format this is equivalent to traversing the term-tree according to a depth-first-search until one reaches a redex."
				+ "<br/> Which abstraction in the displayed term makes up the left part of the redex to reduce first?"
				+ "<br/> You can check your answer by selecting normal order and reducing the term using the Step-Forward-Button.";
		...
		String order2Task = "Applicative Order is the exact opposite of the normal reduction order: the innermost rightmost redex is reduced first. "
		...
		String order3Task = "Call-by-Name reduction is similar to the normal reduction order: the outermost leftmost redex is reduced first."
				+ "<br/> Call-by-Name's major difference is the restriction that only redexes not enclosed in the body of an abstraction may be reduced."
				+ "<br/> This means that when using Call-by-Name a term might still contain redexes but will not be reduced further."
		...
		String order4Task = "Just like Call-by-Name using the Call-by-Value reduction order the outermost leftmost redex is reduced first and redexes enclosed by an abstraction"
				...
				+ "<br/> \"Value\" refers to a lambda term that can not be reduced any further meaning that it is either a variable or an abstraction."
